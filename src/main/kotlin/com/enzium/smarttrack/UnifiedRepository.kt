...
class UnifiedRepository(private val dynamoDbClient: DynamoDbClient) {
    ...
    fun queryEvents(userId: String, limit: Int? = null, sinceTimestamp: Long? = null): List<LifeEvent> {
        ...
        val requestBuilder = QueryRequest.builder().tableName(tableName).keyConditionExpression(condition).expressionAttributeValues(expressionValues).scanIndexForward(false)
    ...
    fun deleteEvent(userId: String, timestamp: Long) {
        try {
            ...
            val query = QueryRequest.builder().tableName(tableName).keyConditionExpression("pk = :pk AND begins_with(sk, :sk)").expressionAttributeValues(mapOf(":pk" to "USER#$userId".toAV(), ":sk" to prefix.toAV())).build()
    ...
    fun getActiveHabits(userId: String): List<Habit> {
        val request = QueryRequest.builder().tableName(tableName).keyConditionExpression("pk = :pk AND begins_with(sk, :sk)").expressionAttributeValues(mapOf(":pk" to "USER#$userId".toAV(), ":sk" to "HABIT#".toAV())).build()
        return try {
            dynamoDbClient.query(request).items().map { mapToHabit(it) }.filter { it.active }
        } catch (e: Exception) {
    ...
    fun archiveHabit(userId: String, habitId: String) {
        ...
        val updateRequest = UpdateItemRequest.builder().tableName(tableName).key(key).updateExpression("SET active = :active").expressionAttributeValues(mapOf(":active" to false.toAV())).build()
